...
uint64_t TCPSender::sequence_numbers_in_flight() const
{
  ...
  return (outstanding_segs.empty()) ? 0 : (abs_seqno - outstanding_segs.front().seqno.unwrap(isn_,abs_seqno));
}
...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  while (true) {
    ...
    message.SYN = (abs_seqno == 0),
    message.RST = input_.has_error();
    size_t bytes_to_read = std::min<size_t>(TCPConfig::MAX_PAYLOAD_SIZE,std::max<size_t>(pretend_window,TCPSender::sequence_numbers_in_flight()) - (message.SYN + TCPSender::sequence_numbers_in_flight()));
    read(input_.reader(), bytes_to_read, message.payload);
    abs_seqno += message.sequence_length();
    message.FIN = ((std::max<size_t>(pretend_window,TCPSender::sequence_numbers_in_flight()) - TCPSender::sequence_numbers_in_flight() - message.sequence_length() > 0) && reader().is_finished() && (abs_seqno == reader().bytes_popped() + 1) );
...
void TCPSender::receive( const TCPReceiverMessage& msg )
{
  ...
  if (msg.ackno.has_value()){
    ...
    while (!outstanding_segs.empty()){
        if (outstanding_segs.front().seqno.unwrap(isn_, ackno) + outstanding_segs.front().sequence_length() <= ackno) {
